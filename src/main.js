...
meetingBtn?.addEventListener('click', async ()=>{
  ...
  `, (root)=>({ url: root.querySelector('#mUrl')?.value?.trim()||'' }), (root)=>{
    ...
    // No extra strategies: always use standard flow on OK
    if (btn){
...
async function openSpaceOptions(space){
  ...
  const body = `
    ...
    <div class='field'>
      ...
      <div id='sharesList' class='muted' style='margin-top:8px; font-size:13px'>Loading shares…</div>
    </div>`;
  const res = await openModalWithExtractor('Space options', body,
    (root)=>({
      name: root.querySelector('#spName')?.value||'',
      vis: root.querySelector('#spVis')?.value||space.visibility||'private',
      color: root.querySelector('#spColors [data-selected="1"]')?.getAttribute('data-color')||'',
      pubPerm: root.querySelector('#spPublicPerm')?.value||'view'
    }),
    async (root)=>{
      // Toggle public permissions on visibility change
      ...
      try{
        ...
        const shares = await mod.db_listShares(space.id).catch(()=>[]);
        const list = root.querySelector('#sharesList');
        if (list){
          if (!shares.length){ list.textContent = 'No shares yet.'; }
          else {
            ...
            list.querySelectorAll('[data-unshare]').forEach(btn=>{
              btn.addEventListener('click', async ()=>{
                try{
                  ...
                  await fetch('/api/spaces-share', { method:'DELETE', headers:{ 'Content-Type':'application/json', ...(token? { Authorization:`Bearer ${token}` }: {}) }, body: JSON.stringify({ space_id: space.id, email: btn.getAttribute('data-unshare') }) });
      ...
      // Invite handler
      const addBtn = root.querySelector('#addShare');
      addBtn?.addEventListener('click', async ()=>{
        ...
        try{
          let token=''; try{ const sb = getSupabase(); const ss = await sb.auth.getSession(); token = ss?.data?.session?.access_token || ''; }catch{}
          const r = await fetch('/api/spaces-share', { method:'POST', headers:{ 'Content-Type':'application/json', ...(token? { Authorization:`Bearer ${token}` }: {}) }, body: JSON.stringify({ space_id: space.id, email, role }) });
          ...
          // Append to list
          const list = root.querySelector('#sharesList');
          if (list){ const div = document.createElement('div'); div.style.cssText='display:flex; align-items:center; justify-content:space-between; gap:8px; border:1px solid var(--border); border-radius:6px; padding:6px 8px; margin-top:6px'; div.innerHTML=`<span>${email} <em style='opacity:.7'>(${role})</em></span><button class='button ghost sm' data-unshare='${email}'>Remove</button>`; list.appendChild(div); }
        }catch(e){ window.showToast && window.showToast('Share failed'); }
      });
    }
  );
  if (!res.ok) return;
  const { name, vis, color, pubPerm } = res.values || {};
  ...
  if (Object.keys(updates).length) await sb.db_updateSpace(space.id, updates);
  // Update public permissions in settings
  if (vis==='public'){
    const settings = { ...(space.settings||{}) };
    settings.public_permissions = pubPerm||'view';
    await sb.db_updateSpace(space.id, { settings });
  }
  renderRoute();
}
