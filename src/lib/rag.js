...
export async function ragIndex(spaceId, items, provider='openai'){
	const anon = util_getEnv('SUPABASE_ANON_KEY','SUPABASE_ANON_KEY');
	if (provider === 'mistral'){
		const r = await fetch('https://lmrnnfjuytygomdfujhs.supabase.co/functions/v1/rag-embed-index', { method:'POST', headers:{ 'Content-Type':'application/json', 'Authorization': `Bearer ${anon}`, 'apikey': anon }, body: JSON.stringify({ space_id: spaceId, items }) });
		return r.json();
	}
