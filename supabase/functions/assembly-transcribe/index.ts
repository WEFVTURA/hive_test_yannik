...
// Supabase Edge Function: assembly-transcribe
// Upload flow (client): upload audio to public bucket -> get public URL -> invoke with { url, space_id, api_key? }
...
serve(async (req: Request) => {
  ...
  // Resolve default space (Meetings) if none provided
  if (!spaceId){
    try{
  ...
  while(Date.now() < deadline){
    ...
    if (status === 'completed'){ transcriptText = st.text || ''; break; }
    if (status === 'error'){ return new Response(JSON.stringify({ error:'transcription_error', detail: st?.error }), { status:500, headers:{...corsHeaders,'Content-Type':'application/json'} }); }
  }
  ...
  let noteId = '';
  try{
    const created = await fetchJson(`${SUPABASE_URL}/rest/v1/notes`, {
      method:'POST',
      headers:{ 'Content-Type':'application/json', apikey: SERVICE_KEY, Authorization: `Bearer ${SERVICE_KEY}` },
      body: JSON.stringify({ space_id: spaceId||null, title, content: transcriptText })
    });
    noteId = created?.[0]?.id || created?.id || '';
  }catch(e){ /* save failure is non-fatal */ }

  return new Response(JSON.stringify({ ok:true, transcript: transcriptText, space_id: spaceId||null, note_id: noteId||null }), { headers:{...corsHeaders,'Content-Type':'application/json'} });
});
